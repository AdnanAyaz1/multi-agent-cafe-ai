      ...
      <div className="p-6">
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[10px] text-zinc-500 uppercase tracking-wider font-mono">Chain of thought</p>
            </div>
          </div>
          <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-amber-500/10 border border-amber-500/20">
            <Loader2 className="w-3 h-3 text-amber-500 animate-spin" />
            <span className="text-[10px] text-amber-500 font-semibold font-mono">RUNNING</span>
          </div>
        </div>
          ...
          {steps.map((step, i) => (
            <div key={step.label} className="relative">
              ...
              <div className="flex items-start gap-4 py-3">
                ...
                {step.status === 'complete' && <span className="text-[9px] text-green-500 font-bold px-2 py-0.5 rounded-full bg-green-500/10 border border-green-500/20 font-mono">DONE</span>}
              </div>
            </div>
          ))}
        </div>
        <div className="flex items-center gap-1.5 flex-wrap mt-4 pt-4 border-t border-zinc-800">
          {AGENT_DISPLAY_ORDER.map((role) => (
            <span key={role} className="text-[9px] px-2.5 py-1 rounded-full border border-zinc-700 text-zinc-400 bg-zinc-800/50 font-mono">{AGENT_CONFIG[role].name}</span>
          ))}
        </div>
      </div>
    </div>
