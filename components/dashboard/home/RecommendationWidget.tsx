        ...
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[10px] text-zinc-400 uppercase tracking-wider">Today&apos;s top actions</p>
            </div>
          </div>
        ...
        <div className="space-y-2">
          {MOCK_RECOMMENDATIONS.map((rec, i) => (
            <div key={i} className="flex items-center gap-3 p-3 rounded-xl bg-white/[0.02] border border-white/[0.04] hover:bg-white/[0.04] hover:border-white/[0.08] transition-all duration-300">
              <div className={`w-7 h-7 rounded-lg flex items-center justify-center flex-shrink-0 ${rec.type === 'auto' ? 'bg-green-500/10 border border-green-500/20' : 'bg-white/[0.04] border border-white/[0.06]'}`}>
                {rec.type === 'auto' ? <Target className="w-3.5 h-3.5 text-green-500" /> : <DollarSign className="w-3.5 h-3.5 text-zinc-400" />}
              </div>
